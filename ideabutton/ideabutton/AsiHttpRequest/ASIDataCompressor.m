...
+ (BOOL)compressDataFromFile:(NSString *)sourcePath toFile:(NSString *)destinationPath error:(NSError **)err
{
    ...
    while ([compressor streamReady]) {
		...
		if ((NSUInteger)[inputStream streamStatus] == (NSUInteger)NSStreamEventErrorOccurred) {
			if (err) {
		...
		// Make sure nothing went wrong
		if ((NSInteger)[inputStream streamStatus] == (NSInteger)NSStreamEventErrorOccurred) {
			if (err) {
